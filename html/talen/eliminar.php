...
$codigo_a_eliminar = $_POST['numerocedula'];

// Consulta SQL para eliminar el registro con el código proporcionado
$sql = "DELETE FROM empleados WHERE numerocedula = '$codigo_a_eliminar'";

// Ejecutar la consulta
if ($conexion->query($sql) === TRUE) {
    $mensaje = "Trabajador eliminado correctamente";
                echo '<script>alert("' . $mensaje . '");</script>';
                echo '<script>window.location.href = "talento.php?usuario=' . urlencode($usuario) . '";</script>';
} else {
